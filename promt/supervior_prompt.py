...
def get_supervior_prompt(members: list[str], current_cruise: dict) -> str:
    return (
        ...
        "If the user ask about the general information, respond with the general_infor worker."
        "If the user ask about add cruise into cart, respond with the add_cart worker."
        "If the user ask about clear context, respond with the clear_context worker."
        "If the Curent cruise is not None and cruise_id is not None and user ask about the list cabin, respond with the cruise_infor worker."
        "If the Curent cruise is None and cruise_id is None and user ask about the list cabin, respond with the general_infor worker."
    )   
