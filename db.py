...
dotenv.load_dotenv()
from utils import enrich_cruise
...
class DBTool:
    ...
    async def get_cruises(self, preferences, currency: str = "USD"):
        ...
        date_conditions = {}
        date_conditions["$gte"] = datetime.strptime(preferences["minSailStartDate"], "%Y-%m-%d").strftime("%Y-%m-%d")
        # date_conditions["$gte"] = datetime.fromisoformat(preferences["minSailStartDate"])

        if preferences.get("maxSailStartDate") is not None:
            date_conditions["$lte"] = datetime.strptime(preferences["maxSailStartDate"], "%Y-%m-%d").strftime("%Y-%m-%d")
        ...
        if preferences.get("minSailEndDate") is not None:
            return_date_conditions["$gte"] = datetime.strptime(preferences["minSailEndDate"], "%Y-%m-%d").strftime("%Y-%m-%d")
        ...
        if preferences.get("maxSailEndDate") is not None:
            return_date_conditions["$lte"] = datetime.strptime(preferences["maxSailEndDate"], "%Y-%m-%d").strftime("%Y-%m-%d")
        ...
        if (preferences.get("destinations", []) is not None) and len(preferences.get("destinations", [])) > 0:
            search_terms = [re.compile(dest.strip(), re.IGNORECASE) for dest in preferences["destinations"]]
            query["$or"] = [
        ...
        if (preferences.get("embarkationPort", []) is not None) and len(preferences.get("embarkationPort", [])) > 0:
            query["embarkationPortName"] = {"$in": preferences["embarkationPort"]}
        if (preferences.get("disembarkationPort", []) is not None) and len(preferences.get("disembarkationPort", [])) > 0:
        ...
        if preferences.get("round_trip") is True:
            query["$expr"] = {"$eq": ["$disembarkationPortName", "$embarkationPortName"]}
        if preferences.get("ignore_destinations") is not None and len(preferences.get("ignore_destinations")) > 0:
            search_terms = [re.compile(dest.strip(), re.IGNORECASE) for dest in preferences.get("ignore_destinations")]
    ...
    def ingest_history(self, session_id, message, sender, cruise_list = []):
        ...
        self.history_collection.update_one({"sessionId": session_id}, {"$set": {"messages": chat_history["messages"]}})
